...
static IRAM_ATTR void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (indev_data.pressed) {
        ...
        /* Original-Invertierung war korrekt fuer das D1-Panel.
         * Rollback: die Werte aus dem Controller kommen gespiegelt,
         * die Normalisierung muss hier passieren. */
    ...
    if (indev_data.pressed) {
        ESP_LOGI(TAG, "TOUCH PRESS: [%3u, %3u]", data->point.x, data->point.y);
    }
}
